...
function addAdventureToDOM(adventures) {
  ...
  adventures.forEach(obj => {
    let {id,name,costPerHead,currency,image,duration,category} = obj;
   dataElem.innerHTML += `
   ...
   <div class="category-banner">${category}</div>
     <div class="card activity-card"> 
        <img src="${image}" />
        <div class="card-body col-md-12 mt-12"> 
...
function filterByDuration(list, low, high) {
 ...
 let filteredList = list.filter(key => key.duration >= low && key.duration <= high);
  return filteredList;
}
...
function generateFilterPillsAndUpdateDOM(filters) {
  ...
  filters["category"].forEach((key) => {
    ...
    let pEle = document.getElementById("category-list");
    let ele = document.createElement("div");
    ele.className = "category-filter";
    ele.innerHTML = key;
